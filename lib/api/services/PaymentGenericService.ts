...
export class PaymentGenericService extends Service {
   ...
   */
  _init(adapter) {
    const Adapter = adapter ? adapter.adapter : this.app.config.get('generics.payment_processor.adapter')
    return new Adapter(adapter ? adapter.options : this.app.config.get('generics.payment_processor.options'))
  }
