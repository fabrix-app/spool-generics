...
export class DataStoreGenericService extends Service {
   ...
   */
  _init(adapter) {
    const Adapter = adapter ? adapter.adapter : this.app.config.get('generics.data_store_provider.adapter')
    return new Adapter(adapter ? adapter.options : this.app.config.get('generics.data_store_provider.options'))
  }
